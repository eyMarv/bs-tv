...
tvsDE_genres_url = (
    "https://raw.githubusercontent.com/sunsettrack4/config_files/master/tvs_genres.json"
)
...
tvsDE_channels_warnings_tmp = os.path.join(
    provider_temppath, "tvsDE_channels_warnings.txt"
)
...
## Channel Files
tvsDE_chlist_provider_tmp = os.path.join(
    provider_temppath, "chlist_tvsDE_provider_tmp.json"
)
...
def select_channels():
    ...
    user_select = channel_selector.select_channels(
        provider, provider_list, selected_list
    )
...
def download_multithread(thread_temppath, download_threads):
    ...
    if filesplit.split_chlist_selected(
        ...
        for file in os.listdir(thread_temppath):
            delete(os.path.join(thread_temppath, file))
    ...
    for user_item in selected_list["channellist"]:
        ...
        with open(broadcast_files, encoding="utf-8") as playbill:
            ...
            for i in range(0, days_to_grab):
                ...
                tvs_data_url = (
                    "https://live.tvspielfilm.de/static/broadcast/list/{}/{}".format(
                        contentID, day_to_grab
                    )
                )
...
def create_xml_broadcast(enable_rating_mapper, thread_temppath, download_threads):
        ...
        try:
            for days in broadcastfiles["broadcasts"]:
                for playbilllist in days:
                    ...
                    item_starrating = ""
                    item_starttime = datetime.utcfromtimestamp(item_starttime).strftime(
                        "%Y%m%d%H%M%S"
                    )
                    item_endtime = datetime.utcfromtimestamp(item_endtime).strftime(
                        "%Y%m%d%H%M%S"
                    )
    ...
    mapper.create_channel_warnings(
        tvsDE_channels_warnings_tmp, tvsDE_channels_warnings, provider, channel_pull
    )
    ...
    mapper.create_genre_warnings(
        tvsDE_genres_warnings_tmp, tvsDE_genres_warnings, provider, genre_pull
    )
    ...
    for file in os.listdir(provider_temppath):
        delete(os.path.join(provider_temppath, file))
