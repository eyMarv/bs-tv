...
from tools import addon_name, addon_version, datapath, loc, log, notify, temppath
...
tkm_genres_url = (
    "https://raw.githubusercontent.com/sunsettrack4/config_files/master/tkm_genres.json"
)
...
## Log Files
magentaDE_genres_warnings_tmp = os.path.join(
    provider_temppath, "magentaDE_genres_warnings.txt"
)
magentaDE_genres_warnings = os.path.join(temppath, "magentaDE_genres_warnings.txt")
magentaDE_channels_warnings_tmp = os.path.join(
    provider_temppath, "magentaDE_channels_warnings.txt"
)
...
def get_epgLength(days_to_grab):
    ...
    calc_today = datetime(
        today.year, today.month, today.day, hour=00, minute=00, second=1
    )
...
## Channel Files
magentaDE_chlist_provider_tmp = os.path.join(
    provider_temppath, "chlist_magentaDE_provider_tmp.json"
)
magentaDE_chlist_provider = os.path.join(
    provider_temppath, "chlist_magentaDE_provider.json"
)
magentaDE_chlist_selected = os.path.join(datapath, "chlist_magentaDE_selected.json")

magentaDE_login_url = (
    "https://api.prod.sngtv.magentatv.de/EPG/JSON/Login?&T=PC_firefox_75"
)
...
magentaDE_channellist_url = (
    "https://api.prod.sngtv.magentatv.de/EPG/JSON/AllChannel?SID=first&T=PC_firefox_75"
)
...
magentaDE_authenticate = {
    ...
    "cnonce": "5c6ff0b9e4e5efb1498e7eaa8f54d9fb",
}
magentaDE_get_chlist = {
    "properties": [
        {
            "name": "logicalChannel",
...
def magentaDE_session():
    ...
    session.post(
        magentaDE_login_url, data=json.dumps(magentaDE_login), headers=magentaDE_header
    )
    session.post(
        magentaDE_authenticate_url,
        data=json.dumps(magentaDE_authenticate),
...
def get_channellist():
    ...
    magenta_chlist_url = session.post(
        magentaDE_channellist_url,
        data=json.dumps(magentaDE_get_chlist),
        headers=magentaDE_header,
    )
    ...
    with open(
        magentaDE_chlist_provider_tmp, "w", encoding="utf-8"
    ) as provider_list_tmp:
    ...
    with open(
        magentaDE_chlist_provider_tmp, "r", encoding="utf-8"
    ) as provider_list_tmp:
    ...
    with open(magentaDE_chlist_provider, encoding="utf-8") as provider_list:
        ...
        for channels in magentaDE_channels["channellist"]:
            ...
            y = {"contentId": ch_id, "name": ch_title, "pictures": [{"href": hdimage}]}
...
def download_multithread(thread_temppath, download_threads):
    ...
    with open(magentaDE_chlist_selected, "r", encoding="utf-8") as s:
        selected_list = json.load(s)
    if filesplit.split_chlist_selected(
        thread_temppath,
        magentaDE_chlist_selected,
        splitname,
        download_threads,
        enable_multithread,
    ):
        ...
        for j in jobs:
            while j.is_alive():
                ...
                percent_remain = int(100) - int(items) * int(100) / int(
                    items_to_download
                )
        ...
        for file in os.listdir(thread_temppath):
            tools.delete(os.path.join(thread_temppath, file))

    else:
        multi = False
        log(
            "{} {} ".format(
                provider, "Can`t download in Multithreading mode, loading single..."
            )
        )
...
def download_thread(magentaDE_chlist_selected, multi, list, starttime, endtime):
    ...
    for user_item in selected_list["channellist"]:
        ...
        response = session.post(
            magentaDE_data_url,
            data=json.dumps(magentaDE_data),
            headers=magentaDE_header,
        )
        response.raise_for_status()
        tkm_data = response.json()
        broadcast_files = os.path.join(
            provider_temppath, "{}_broadcast.json".format(contentID)
        )
...
def create_xml_channels():
    ...
    for user_item in selected_list["channellist"]:
        ...
        ## Map Channels
        if not channel_id == "":
            channel_id = mapper.map_channels(
                channel_id,
                channel_format,
                tkm_channels_json,
...
def create_xml_broadcast(enable_rating_mapper, thread_temppath, download_threads):
    ...
    items = 0
    notify(
        "{} {} ".format(loc(32503), provider), "{} Prozent verbleibend".format("100")
    )
        ...
        try:
            for playbilllist in broadcastfiles["playbilllist"]:
                ...
                if not item_starttime == "" and not item_endtime == "":
                    ...
                    item_starttime = (
                        start[0].replace(" ", "").replace("-", "").replace(":", "")
                    )
                    stop = item_endtime.split(" UTC")
                    item_endtime = (
                        stop[0].replace(" ", "").replace("-", "").replace(":", "")
                    )
                if not item_country == "":
    ...
    mapper.create_channel_warnings(
        magentaDE_channels_warnings_tmp,
        magentaDE_channels_warnings,
        provider,
        channel_pull,
    )
    ...
    mapper.create_genre_warnings(
        magentaDE_genres_warnings_tmp, magentaDE_genres_warnings, provider, genre_pull
    )
    ...
    if os.path.isfile(magentaDE_channels_warnings) or os.path.isfile(
        magentaDE_genres_warnings
    ):
    ...
    for file in os.listdir(provider_temppath):
        tools.delete(os.path.join(provider_temppath, file))
