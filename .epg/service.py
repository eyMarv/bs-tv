...
def run_grabber():
    ...
    if check_startup():
        ...
        xml_structure.xml_channels(
            "PULS24",
            "PULS24",
            "https://upload.wikimedia.org/wikipedia/commons/thumb/2/2f/PULS24logo.png/640px-PULS24logo.png",
            "de",
        )
        xml_structure.xml_channels(
            "ServusTV",
            "ServusTV",
            "https://upload.wikimedia.org/wikipedia/commons/thumb/3/37/ServusTV_Logo.png/640px-ServusTV_Logo.png",
            "de",
        )
        ...
        if check_channel_dupes():
            ...
            time_end = str(
                (datetime.now() + timedelta(days=2)).strftime("%Y-%m-%dT%H:%M:00.000Z")
            )
            ...
            epg_post = json.dumps(
                {"platformCodename": "www", "from": time_start, "to": time_end}
            )
            epg_resp = requests.post(
                epg_url,
                timeout=5,
                headers=api_headers,
                data=epg_post,
            ...
            for program in epg_data:
                item_starttime = datetime.strptime(
                    program["start"].split("+")[0], "%Y-%m-%dT%H:%M:%S"
                ).strftime("%Y%m%d%H%M%S")
                item_endtime = datetime.strptime(
                    program["stop"].split("+")[0], "%Y-%m-%dT%H:%M:%S"
                ).strftime("%Y%m%d%H%M%S")
                items_genre = (
                    program["categories"][-1]["name"]
                    if len(program.get("categories", [])) > 0
                    else ""
                )
                item_country = (
                    ", ".join([i["name"] for i in program["countries"]])
                    if len(program.get("countries", [])) > 0
                    else ""
                )
                ...
                if len(program.get("people", [])) > 0:
                    ...
                    items_actor = "; ".join(
                        [i + ": " + ", ".join([a for a in c[i]]) for i in c.keys()]
                    )
                else:
                    items_actor = ""
