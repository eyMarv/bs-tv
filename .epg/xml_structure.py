...
def xml_broadcast(
    ...
    if not item_starttime == "" and not item_endtime == "" and not item_title == "":
        ...
        if not item_starttime == "" and not item_endtime == "":
            ...
            guide.append(
                '    <programme start="{} +0000" stop="{} +0000" channel="{}">\n'.format(
                    item_starttime, item_endtime, channel_id
                )
            )
        ...
        if not item_subtitle == "":
            item_subtitle = (
                item_subtitle.replace("&", "&amp;")
                .replace("<", "&lt;")
                .replace(">", "&gt;")
            )
            guide.append(
                '        <sub-title lang="{}">{}</sub-title>\n'.format(
                    lang, item_subtitle
                )
            )
        ...
        if not item_description == "":
            ...
            ## Rating Mapper
                ...
                desc = '<desc lang="{}">{} {} {} {} {} {}'.format(
                    lang, country, date, season, episode, fsk, imdbstars
                )
                guide.append(
                    "        {}\n        {}</desc>\n".format(
                        " ".join(desc.split()), item_description
                    )
                )
        ...
        # Producer + Director
            ...
            not items_director == "" and not items_producer == "" and items_actor == ""
        ):
            guide.append("        <credits>" + "\n")
            for director in directorlist:
                guide.append("            <director>{}</director>\n".format(director))
        ...
        # Director + Actor
            ...
            not items_director == "" and items_producer == "" and not items_actor == ""
        ):
            guide.append("        <credits>\n")
            for director in directorlist:
                guide.append("            <director>{}</director>\n".format(director))
        ...
        # Producer + Actor
            ...
            items_director == "" and not items_producer == "" and not items_actor == ""
        ):
            guide.append("        <credits>\n")
            for actor in actorlist:
                guide.append("            <actor>{}</actor>\n".format(actor))
            ...
            if not item_season == "" and not item_episode == "":
                ...
                guide.append(
                    '        <episode-num system="xmltv_ns">{} . {} . </episode-num>\n'.format(
                        str(item_season_ns), str(item_episode_ns)
                    )
                )
