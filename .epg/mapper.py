...
def map_genres(items_genre, genre_format, genres_json, genres_warnings_tmp, lang):
    if genre_format == "eit":
        with open(genres_json, "r", encoding="utf-8") as c:
...
def map_channels(
    channel_id, channel_format, channels_json, channels_warnings_tmp, lang
):
...
def create_channel_warnings(
    channels_warnings_tmp, channels_warnings, provider, channel_pull
):
